...
export default function CompleteFormExample(){
    const [coptions,setoptions] = useState({
        name:"",subject:window.location.pathname.split('/')[2],topic:""
    })
        ...
        <ListGroupItem className="p-3">
            ...
            <Row form>
                  <Col md="12" className="form-group">
                    <label htmlFor="feEmailAddress">Chapter</label>
                    <FormInput
                      onChange={(e)=>{addOptions("name",e.target.value)}}
                      id="feEmailAddress"
                      type="text"
                ...
                <Row form>
                  <Col md="12" className="form-group">
                    ...
                    <FormSelect id="feInputState"
                      onChange={(e)=>{addOptions("topic",e.target.value)}}

                    >
                      {topics?topics.map((data,idx)=>{
                      return (
                        <option>
                          {data.name}
                        </option>
